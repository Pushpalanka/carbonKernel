...
public class GhostDeployer extends AbstractDeployer {
    ...
    public void deploy(DeploymentFileData deploymentFileData) throws DeploymentException {
        ...
        if (deployer == null) {
            //HACK to fix CARBON-15041 for Carbon 4.2.0
            directoryName = (String) CarbonContext.getThreadLocalCarbonContext().getProperty("DEPLOYER_DIRECTORY");
            fileExtension = (String) CarbonContext.getThreadLocalCarbonContext().getProperty("DEPLOYER_EXTENSION");
            deployer = getDeployer(directoryName, fileExtension);
    ...
    public void undeploy(String fileName) throws DeploymentException {
        ...
        if(deployer == null ) {
            //HACK to fix CARBON-15041 for Carbon 4.2.0
            directoryName = (String) CarbonContext.getThreadLocalCarbonContext().getProperty("DEPLOYER_DIRECTORY");
            extension = (String) CarbonContext.getThreadLocalCarbonContext().getProperty("DEPLOYER_EXTENSION");
            deployer = getDeployer(directoryName, extension);
        }
    ...
    private String calculateDirectoryName(String servicePathStr) {
        ...
        /* in the case of tenants, to compensate for the error of dropping the drive name
        (ex: C:). File name and repo path being in two different formats in windows
          */
        File repoPathUrlToFile = new File(repoPath);
        repoPath = repoPathUrlToFile.getAbsolutePath();
        repoPath = GhostDeployer.separatorsToUnix(repoPath);
